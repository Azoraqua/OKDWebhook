...
object WebhookUtils {
            ...
            FileInputStream(plugin.configFile).use { inputStream ->
                ...
                webhooksSection.forEach { (webhookKey, webhookValue) ->
                    (webhookValue as? Map<*, *>)?.let { webhookMap ->
                        ...
                        plugin.webhooks.add(Webhook(name, url, messageTemplate, embedTemplate))
     ...
     */
    fun <P : IPlugin> sendToWebhook(plugin: P, webhook: Webhook, payload: String? = null) =
        CoroutineScope(Dispatchers.IO).launch {
            ...
            try {
                ...
                if (isDiscordWebhook) {
                    ...
                    webhook.embedTemplate?.let { template ->
                        template.title?.let {
    ...
    fun requiresMessage(webhook: Webhook): Boolean {
        return webhook.messageTemplate?.contains("%message%") == true ||
                webhook.embedTemplate?.description?.contains("%message%") == true
    }
}